	...
	const FardanDev = WAConnection({
        ...
        generateHighQualityLinkPreview: true,     
         ...
         if (store) {
           ...
           return {
          conversation: 'WhatsApp Bot By FardanDev'
         }}		
	})
...
//================================================================================
	...
	if (pairingCode && !
FardanDev.authState.creds.registered) {
		let phoneNumber;
	    phoneNumber = await question(chalk.black(chalk.white.bold("\n\u0023\u002d\u0020\u0043\u0072\u0065\u0064\u0069\u0074\u0073\u0020\u0042\u0079\u0020\u0053\u006b\u0079\u007a\u006f\u0070\u0065\u0064\u0069\u0061\n"), chalk.red.bold("\u0043\u006f\u006e\u0074\u0061\u0063\u0074\u0020\u003a\u0020\u0077\u0061\u002e\u006d\u0065\u002f\u0036\u0032\u0038\u0035\u0036\u0032\u0034\u0032\u0039\u0037\u0038\u0039\u0033\n\n"), chalk.magenta.bold(`${imageAscii}`), chalk.magenta.italic(`\n\n# Masukan Nomor WhatsApp,\nContoh Format Number +6285XXX\n`)))
			...
			let code = await FardanDev.requestPairingCode(phoneNumber);
...
await store.bind(FardanDev.ev)	
await Solving(FardanDev, store)
	
//================================================================================
	
FardanDev.ev.on('creds.update', await saveCreds)

//================================================================================

FardanDev.ev.on('connection.update', async (update) => {
...
FardanDev.end(`Unknown DisconnectReason : ${reason}|${connection}`)
			}
		}
		if (connection == 'open') {
FardanDev.sendMessage(FardanDev.user.id.split(":")[0] + "@s.whatsapp.net", {text: `${`*#- Simple Botz Version 3.0.0*

Don't forget to subscribe to the YouTube developer -> https://youtube.com/@skyzodev so you can get the latest updates about this script :)`.toString()}`})

try {
FardanDev.newsletterFollow(String.fromCharCode(49, 50, 48, 51, 54, 51, 50, 53, 55, 48, 51, 51, 57, 56, 56, 52, 57, 54, 64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114))
} catch (e) {
}
...
FardanDev.ev.on('messages.upsert', async (message) => {
 await MessagesUpsert(FardanDev, message, store);
});

//================================================================================

FardanDev.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let id = 
FardanDev.decodeJid(contact.id)
...
FardanDev.ev.on('group-participants.update', async (update) => {
const { id, author, participants, action } = update
  ...
  if (global.db.groups[id] && global.db.groups[id].welcome == true) {
    const metadata = await FardanDev.groupMetadata(id)
    let teks
    for(let n of participants) {
      let profile;
      try {
        profile = await FardanDev.profilePictureUrl(n, 'image');
      } catch {
      ...
      if(action == 'add') {
        ...
        let asu = await FardanDev.sendMessage(id, {
          text: `${teks}`,
          mentions: [author, n]
        }, {
          quoted: qtext
        })
await FardanDev.relayMessage(id, {
  "productMessage": {
    "product": {
      "productImage": imguser.imageMessage, 
      "productId": "343056591714248",
      "title": "Welcome To Group",
      "description": `Selamat datang @${FardanDev.getName(n)}`,
      "productImageCount": 1
    },
    "businessOwnerJid": "6285624297893@s.whatsapp.net",
    "contextInfo": {
      mentionedJid: [n]
    }
  }
  ...
  "productMessage": {
    "product": {
      ...
      "description": `Selamat tinggal @${FardanDev.getName(n)}`,
      "productImageCount": 1
    },
    "businessOwnerJid": "6285624297893@s.whatsapp.net",
    "contextInfo": {
  ...
  "productMessage": {
    "product": {
      ...
      "description": `Promote member @${FardanDev.getName(n)}`,
      "productImageCount": 1
    },
    "businessOwnerJid": "6285624297893@s.whatsapp.net",
    "contextInfo": {
  ...
  "productMessage": {
    "product": {
      ...
      "productId": "343056591714248",
      "title": "Demote Member",
      "description": `Demote member @${FardanDev.getName(n)}`,
      "productImageCount": 1
    },
    "businessOwnerJid": "6285624297893@s.whatsapp.net",
    "contextInfo": {
...
FardanDev.ev.on('groups.update', async (update) => {
		try {
		const data = update[0]
		const qtext = {
    key: {
    ...
    message: {
      "extendedTextMessage": {
		...
		if (data?.inviteCode) {      
		let botNumber = FardanDev.user.id.split(":")[0]
		let participant = data.author
		if (participant.split("@")[0] === botNumber) return      
  await FardanDev.sendMessage(data.id, {text: `@${participant.split("@")[0]} telah *mereset* link grup`, mentions: [participant]}, {quoted: qtext})
		}
		...
		let participant = data.author
		if (participant.split("@")[0] === botNumber) return      
		await FardanDev.sendMessage(data.id, {text: `@${participant.split("@")[0]} telah *memperbarui* deskripsi grup`, mentions: [participant]}, {quoted: qtext})
		}
		...
		let participant = data.author
		if (participant.split("@")[0] === botNumber) return      
		await FardanDev.sendMessage(data.id, {text: `@${participant.split("@")[0]} telah *mengganti* nama grup`, mentions: [participant]}, {quoted: qtext})
		}		
